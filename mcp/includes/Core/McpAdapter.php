...
final class McpAdapter {
	...
	private function maybe_create_default_server(): void {
		...
		// wp_abilities_api_categories_init and wp_abilities_api_init fire at
		// rest_api_init priority 10.  McpAdapter::init() runs at priority 15, so
		// by the time we reach here those hooks have ALREADY fired.  Using plain
		// add_action() would register callbacks that never run.
		//
		// Fix: guard with did_action().  If the hook fired, call the method
		// directly.  If it hasn't fired yet (e.g. WP-CLI or a very early init
		// path), attach normally so it runs at the right time.
