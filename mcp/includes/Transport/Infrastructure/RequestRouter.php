...
class RequestRouter {
	...
	private function handle_initialize_with_session( array $params, $request_id, ?HttpRequestContext $http_context, ?string &$new_session_id = null ): AbstractDataTransferObject {
		...
		// Per MCP spec 2025-11-25 §3.3, initialize MUST always create a new session
		// and return a new Mcp-Session-Id, even if the request carried an existing
		// session ID header.  The old guard `! $http_context->session_id` caused
		// reconnecting clients (who re-send initialize with their stale session ID)
		// to receive no new session ID, leaving them stuck with an expired one (Issue 3).
		if ( $http_context ) {
			// Extract the negotiated version from the result so it can be persisted
			// in the session.  Avoids needing to re-negotiate on every reconnect.
			$init_array         = $init_result->toArray();
			$negotiated_version = $init_array['protocolVersion'] ?? null;

			// Extract clientInfo (name + version) from the initialize params so it
			// is stored alongside the session for the lifetime of the connection.
			$client_info = ( isset( $params['clientInfo'] ) && is_array( $params['clientInfo'] ) )
				? $params['clientInfo']
				: null;

			$session_result = HttpSessionValidator::create_session( $params, $negotiated_version, $client_info );
