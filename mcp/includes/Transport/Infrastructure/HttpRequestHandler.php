...
class HttpRequestHandler {
	...
	/**
	 * Mcp-Session-Id value to attach to the current response, or null.
	 *
	 * Stored as an instance property instead of via add_filter() to avoid
	 * filter-closure accumulation in long-lived PHP-FPM worker processes
	 * (Issues 4, 15).  Reset at the top of each process_mcp_messages() call.
	 *
	 * @var string|null
	 */
	private ?string $pending_session_id = null;

	/**
	 * Mcp-Session-Expires value (ISO 8601) to attach to the current response.
	 *
	 * Same rationale as $pending_session_id (Issue 5).
	 *
	 * @var string|null
	 */
	private ?string $pending_session_expires = null;
	...
	public function handle_request( HttpRequestContext $context ): \WP_REST_Response {
		...
		// Handle PATCH requests (session renewal / heartbeat).
		// Clients send PATCH to reset the inactivity timer without a tool call,
		// keeping long-lived connections alive during idle periods.
		if ( 'PATCH' === $context->method ) {
			return $this->handle_session_renewal( $context );
		}
	...
	private function process_mcp_messages( HttpRequestContext $context ): \WP_REST_Response {
		// Reset pending headers for this request cycle.
		// Instance properties survive PHP-FPM worker reuse; clearing them here
		// ensures no stale header values bleed across requests (Issues 4, 5, 15).
		$this->pending_session_id      = null;
		$this->pending_session_expires = null;
		...
		if ( null === $response_body ) {
			...
			// Explicitly set Content-Type — some reverse proxies (nginx) strip or
			// override it for responses with no body (Issue 7).
			$response->header( 'Content-Type', 'application/json' );
			return $this->apply_pending_headers( $response );
		}
		...
		if ( ! $is_batch_request && isset( $response_body['error'] ) ) {
			...
			return $this->apply_pending_headers( new \WP_REST_Response( $response_body, $http_status ) );
		}

		return $this->apply_pending_headers( new \WP_REST_Response( $response_body, 200 ) );
	}

	/**
	 * Apply any pending MCP headers to a response object and return it.
	 *
	 * Called from every return path in process_mcp_messages() so headers are
	 * always set directly on the WP_REST_Response rather than via add_filter()
	 * (which accumulated stale closures in PHP-FPM worker processes).
	 *
	 * @param \WP_REST_Response $response
	 * @return \WP_REST_Response
	 */
	private function apply_pending_headers( \WP_REST_Response $response ): \WP_REST_Response {
		if ( null !== $this->pending_session_id ) {
			$response->header( 'Mcp-Session-Id', $this->pending_session_id );
		}
		if ( null !== $this->pending_session_expires ) {
			$response->header( 'Mcp-Session-Expires', $this->pending_session_expires );
		}
		return $response;
	}
	...
	private function process_jsonrpc_request( array $message, HttpRequestContext $context ): array {
		...
		// Validate session for all requests except initialize (router handles initialize session creation).
		if ( 'initialize' !== $method ) {
			...
			// Emit Mcp-Session-Expires header so clients know when to renew.
			$user_id = get_current_user_id();
			if ( $user_id && $context->session_id ) {
				$expiry = SessionManager::get_session_expiry_time( $user_id, $context->session_id );
				if ( false !== $expiry ) {
					$this->add_session_expiry_header( $expiry );
				}
			}
		}
	...
	/**
	 * Queue the Mcp-Session-Id header for the current response.
	 *
	 * Previously used add_filter('rest_post_dispatch') with a static variable
	 * guard.  Static variables persist across PHP-FPM worker requests, causing
	 * stale session IDs from earlier requests to appear in later ones, and
	 * causing filter closures to accumulate indefinitely in $wp_filter (Issues
	 * 4 and 15).  Now stores the value in an instance property that is reset at
	 * the top of process_mcp_messages() on every request.
	 *
	 * @param string $session_id The session ID to include in the response.
	 * @return void
	 */
	private function add_session_header_to_response( string $session_id ): void {
		$this->pending_session_id = $session_id;
	}
	 ...
	 * Handle PATCH requests (session renewal / heartbeat).
	 *
	 * Clients can PATCH the MCP endpoint with their Mcp-Session-Id header to
	 * reset the inactivity timer without running a tool call.  This is useful
	 * for AI desktop apps that stay open for days but only query occasionally.
	 *
	 * Response: HTTP 200 with a JSON body containing the new expiry timestamp
	 * so the client can schedule its next heartbeat.
	 *
	 * @param \WP\MCP\Transport\Infrastructure\HttpRequestContext $context The HTTP request context.
	 *
	 * @return \WP_REST_Response Renewal response.
	 */
	private function handle_session_renewal( HttpRequestContext $context ): \WP_REST_Response {
		$result = HttpSessionValidator::renew_session( $context );
		...
		// Return the new expiry time so clients can schedule their next heartbeat.
		$user_id = get_current_user_id();
		$expiry  = ( $user_id && $context->session_id )
			? SessionManager::get_session_expiry_time( $user_id, $context->session_id )
			: false;
	...
	/**
	 * Queue the Mcp-Session-Expires header for the current response.
	 *
	 * Previously used add_filter('rest_post_dispatch') with a static variable
	 * guard — same PHP-FPM accumulation problem as add_session_header_to_response
	 * (Issue 5).  Now stores in an instance property cleared per request.
	 *
	 * @param int $expiry_timestamp Unix timestamp of session expiry.
	 * @return void
	 */
	private function add_session_expiry_header( int $expiry_timestamp ): void {
		$this->pending_session_expires = gmdate( 'c', $expiry_timestamp );
	}
}
