...
class McpErrorFactory {
	...
	public static function mcp_error_to_http_status( $mcp_error_code ): int {
		...
		switch ( $code ) {
			...
			// Transport-level not-found errors (the resource/session genuinely does not exist).
			...
			// METHOD_NOT_FOUND (-32601) is a JSON-RPC application-level error:
			// the request was well-formed but the method name is unknown.  Per
			// JSON-RPC 2.0 spec the server must return HTTP 200 with a JSON-RPC
			// error body — NOT 404.  Returning 404 causes HTTP clients and proxies
			// to treat it as "endpoint not found" and retry or surface wrong UI
			// (Issue 6).
			// Falls through to the default (return 200) branch below.
